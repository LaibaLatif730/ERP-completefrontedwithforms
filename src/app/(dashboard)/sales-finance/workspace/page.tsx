    ...
    <div className="p-6 space-y-6  bg-gray-50 dark:bg-black text-black dark:text-white min-h-screen  text-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {actions.map((item, idx) => (
          <button
            key={idx}
            className="flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-orange-500 hover:bg-orange-600 text-white shadow transition-all text-sm"
          >
        ...
        <CardContent className="h-[300px] text-sm">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={salesData} margin={{ top: 10, right: 30, left: 0, bottom: 5 }}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="ProductA" fill="#a5f3fc" /> {/* Blue-300 */}
              <Bar dataKey="ProductB" fill="#fcd34d" /> {/* Pink-300 */}
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="p-4">
          ...
          <CardContent className="space-y-2 text-sm">
            <p>⚠️ Invoice #1080 is 30 days overdue</p>
